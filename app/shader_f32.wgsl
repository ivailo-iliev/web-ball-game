...
fn dom_norm(rgb: vec3<f32>, colorIdx: u32) -> f32 {
  const EPS: f32 = 0.001;
...
fn cross5_rgb(p: vec2<i32>) -> vec3<f32> {
  ...
  return (c0 + c1 + c2 + c3 + c4) * (1.0 / 5.0);
}

// 4-tap ring mean of *normalized dominance* at +/-x, +/-y with integer radius (no trig)
fn ring_dom4_norm(center: vec2<i32>, radius: i32, colorIdx: u32) -> f32 {
...
fn seed_grid(
  ...
  const SEED_CTHR : f32 = 0.025; // default
  ...
  if (activeA) {
    ...
    let passA  = (sat >= f32(U.satMinA)) &&
                 (domA >= f32(U.domThrA)) &&
                 (y >= f32(U.yMinA)) && (y <= f32(U.yMaxA));
    let scoreA = select(0.0, domA - ring_dom4_norm(p, iR, U.colorA), passA);
    if (scoreA >= SEED_CTHR) {
      // tiny preview mark at the sample
      textureStore(maskOut, vec2<i32>(i32(sx), i32(sy)), vec4<f32>(color_from_index(U.colorA), 1.0));
      let key  = pack_key(f32(clamp(scoreA, 0.0, 1.0)), idx);
  ...
  if (activeB) {
    ...
    let passB  = (sat >= f32(U.satMinB)) &&
                 (domB >= f32(U.domThrB)) &&
                 (y >= f32(U.yMinB)) && (y <= f32(U.yMaxB));
    let scoreB = select(0.0, domB - ring_dom4_norm(p, iR, U.colorB), passB);
    if (scoreB >= SEED_CTHR) {
      textureStore(maskOut, vec2<i32>(i32(sx), i32(sy)), vec4<f32>(color_from_index(U.colorB), 1.0));
      let key  = pack_key(f32(clamp(scoreB, 0.0, 1.0)), idx);
...
fn refine_micro(
  ...
  if (processA && inBounds && sampleThis) {
    ...
    if (dxA*dxA + dyA*dyA <= Rr2) {
      ...
      if ((sat >= f32(U.satMinA)) && (dA >= f32(U.domThrA)) && (y >= f32(U.yMinA)) && (y <= f32(U.yMaxA))) {
        atomicAdd(&wgCntA,  1u);
  ...
  if (processB && inBounds && sampleThis) {
    ...
    if (dxB*dxB + dyB*dyB <= Rr2) {
      ...
      if ((sat >= f32(U.satMinB)) && (dB >= f32(U.domThrB)) && (y >= f32(U.yMinB)) && (y <= f32(U.yMaxB))) {
        atomicAdd(&wgCntB,  1u);
